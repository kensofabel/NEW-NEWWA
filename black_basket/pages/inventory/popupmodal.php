...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!price) price = 'variable';
            ...
            var inStock = document.getElementById('inlineInStock').value.trim() || '0';
            var lowStock = document.getElementById('inlineLowStock').value.trim() || '0';
            ...
            if (variantsTable) {
                variantsTable.querySelectorAll('tr').forEach(function(row) {
                    var variant = {
                        name: row.querySelector('.variant-name input') ? row.querySelector('.variant-name input').value.trim() : '',
                        price: row.querySelector('.variant-price input') ? row.querySelector('.variant-price input').value.replace(/[^\d.]/g, '') : '',
                        cost: row.querySelector('.variant-cost input') ? row.querySelector('.variant-cost input').value.replace(/[^\d.]/g, '') : '',
                        in_stock: row.querySelector('.variant-instock input') ? row.querySelector('.variant-instock input').value.trim() : '0',
                        low_stock: row.querySelector('.variant-lowstock input') ? row.querySelector('.variant-lowstock input').value.trim() : '0',
                        sku: row.querySelector('.variant-sku input') ? row.querySelector('.variant-sku input').value.trim() : '',
                        barcode: row.querySelector('.variant-barcode input') ? row.querySelector('.variant-barcode input').value.trim() : '',
                        pos_available: row.querySelector('.variant-pos input') ? (row.querySelector('.variant-pos input').checked ? 1 : 0) : 1
                    };
