...
switch ($method) {
    ...
    $in_stock = isset($data['in_stock']) ? trim($data['in_stock']) : '';
    if ($in_stock === '' || $in_stock === '0 - -' || $in_stock === '0') {
        $in_stock = '0';
    }
    $low_stock = isset($data['low_stock']) ? trim($data['low_stock']) : '';
    if ($low_stock === '' || $low_stock === '0 - -' || $low_stock === '0') {
        $low_stock = '';
    }
    ...
    $price_sql = ($price === 'variable') ? "'variable'" : ($price !== null ? $price : 'NULL');
        ...
        try {
            ...
            foreach ($variants as $variant) {
                ...
                // Accept in_stock and low_stock as VARCHAR (with unit suffix)
                $vin_stock = isset($variant['in_stock']) ? trim($variant['in_stock']) : '';
                if ($vin_stock === '' || $vin_stock === '0 - -' || $vin_stock === '0') {
                    $vin_stock = '0';
                }
                $vlow_stock = isset($variant['low_stock']) ? trim($variant['low_stock']) : '';
                if ($vlow_stock === '' || $vlow_stock === '0 - -' || $vlow_stock === '0') {
                    $vlow_stock = '';
                }
